const fs = require('fs');
const data = `1,"Mar 28, Sat",7:30 PM,Royal Challengers Bengaluru vs Sunrisers Hyderabad,"M. Chinnaswamy Stadium, Bengaluru"
...
console.log('export const MATCH_SCHEDULE = [');
...
for (const line of lines) {
    ...
    const [_, idStr, dateRaw, timeRaw, teamsRaw, venueRaw] = match;
    const matchId = `m${idStr}`;
    
    // date: "Mar 28, Sat" -> "Mar 28, 2026"
    const monthDay = dateRaw.split(',')[0].trim();
    const fullDate = `${monthDay}, 2026`;
    
    // time: "7:30 PM" -> "19:30"
    const [timeVal, ampm] = timeRaw.trim().split(' ');
    ...
    // teams 
    const [teamAInfo, teamBInfo] = teamsRaw.split(' vs ').map(t => t.trim());
    
    const teamAShort = teamMap[teamAInfo] || '';
    const teamBShort = teamMap[teamBInfo] || '';
    
    console.log(`  { id: '${matchId}', teamA: { name: '${teamAInfo}', short: '${teamAShort}' }, teamB: { name: '${teamBInfo}', short: '${teamBShort}' }, date: '${fullDate}', time: '${formattedTime}', venue: '${venueRaw}', league: 'TATA IPL 2026' },`);
}
